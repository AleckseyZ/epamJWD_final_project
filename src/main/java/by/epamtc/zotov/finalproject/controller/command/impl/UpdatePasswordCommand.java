...
public class UpdatePasswordCommand implements Command {
    ...
    public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (Validator.getInstance().validateNumber(targetId) && Validator.getInstance().validatePassword(password)
                && Validator.getInstance().validatePassword(newPassword) && newPassword.equals(repeatPassword)) {
            try {
                if (ServiceFactory.getInstance().getUserService().updatePassword(Integer.parseInt(targetId),
                        password.toCharArray(), newPassword.toCharArray(), repeatPassword.toCharArray())) {
                    isSuccessful = true;
                }
        ...
        response.sendRedirect(CommandPaths.LOG_OUT);
    }
}