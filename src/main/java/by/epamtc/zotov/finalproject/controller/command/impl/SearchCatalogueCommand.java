            ...
            try {
                switch (criteria) {
                    case JSPAtributes.TITLE:
                        books = ServiceFactory.getInstance().getBookService().findBooksByTitle(searchString);
                        break;
                    case JSPAtributes.AUTHOR:
                        books = ServiceFactory.getInstance().getBookService().findBooksByAuthor(searchString);
                        break;
                    case JSPAtributes.TAGS:
                        books = ServiceFactory.getInstance().getBookService().findBooksByTags(searchString);
                        break;
                }
