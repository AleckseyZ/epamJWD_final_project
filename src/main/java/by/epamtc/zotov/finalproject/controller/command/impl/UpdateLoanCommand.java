...
public class UpdateLoanCommand implements Command {
    ...
    public void execute(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        if (Validator.getInstance().validateNumber(targetId) && status != null) {
            ...
            try {
                if (ServiceFactory.getInstance().getLoanService().updateLoan(targetId, status)) {
                    isSuccessful = true;
                }
        ...
        if (request.getSession().getAttribute(JSPAtributes.ROLE).equals(JSPAtributes.USER_ROLE)) {
